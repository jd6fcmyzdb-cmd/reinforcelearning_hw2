...
def rollout_trajectory(
    ...
    while True:
        ...
        # check output action matches the action space
        ...
        # truncateds.append(truncated or steps > max_length)
...
def rollout_trajectory_robotics(
    env: gym.Env, policy: MLPPolicy, max_length: int, render: bool = False
) -> Dict[str, np.ndarray]:
    """Sample a rollout in the environment from a policy."""
    ts, _ = env.reset()
...
def rollout_trajectories(
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        # collect rollout
        if robotics:
            traj = rollout_trajectory_robotics(env, policy, max_length, render)
        else:
